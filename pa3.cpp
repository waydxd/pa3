  ...
  if(search_course(course_array, course_id, num_courses,i)){return false;}
    ...
    for(int i = 0;i<num_courses;i++){
      if(course_array==nullptr){
        ...
        course_array[i]->name == name;
        course_array[i]->star_rank_head == nullptr;
        course_array[i]->stars_count == 0;
        return true;
      }
    }
    num_courses *= 2;
    ...
    new_course_array[num_courses/2]->name == name;
    new_course_array[num_courses/2]->star_rank_head == nullptr;
    new_course_array[num_courses/2]->stars_count == 0;
    delete[] course_array;
...
bool add_star_rank(Student *&student_head, unsigned int sid,
  ...
  StarRank *star_prev = nullptr;
  StarRank *starCurrent = course_array[i]->star_rank_head;
  if(search_star_rank(course_array[i]->star_rank_head,sid,star_prev,starCurrent)){
    cout << "Failed to insert star_rank because the student " << sid
         << " already have a star_rank." << endl;
  }
  //insert the star_rank
  else{
    ...
    if(star_prev == nullptr){
      new_star_rank->next = course_array[i]->star_rank_head;
      course_array[i]->star_rank_head = new_star_rank;
    }
    else{
      star_prev->next = new_star_rank;
      new_star_rank->next = starCurrent;
    }
    course_array[i]->stars_count[star-1]++;
    current->ranks_count++;
    return true;
  }
  return false;
...
bool add_student(Student *&student_head, const unsigned int sid,
  ...
  prev = nullptr;
  if(search_student(student_head, sid, prev, current)){return false;}
  else{
    Student *new_student = new Student;
    new_student->sid = sid;
    strcpy(new_student->name, name);
  ...
  if(course_array==nullptr){
    ...
    for(int i = 0; i<num_courses; i++){
      ...
      for(int j = 0; j<MAX_RANKING_STARS;j++){
        ...
        cout << course_array[i]->stars_count[j] << endl;
      }
    }
  }
}
